...
asignatura('HRW-132', 'Humanistico 1', 5, 1, 4).
...
asignatura('HRW-101', 'Humanistico 2', 3, 2, 10).
...
asignatura('FIS-120', 'Fisica General 2', 9, 4, 20).
...
prerrequisitos('INF-288', ['INF-360']).
prerrequisitos('INF-309', ['INF-360']).

%semestre 11
prerrequisitos('INF-310', ['INF-288', 'INF-309']).
...
%parametro 2: lista de ramos aprobados
%descripcion: dadas las 2 listas verifica si la lista de prerrequisitos se encuentra dentro de la de aprobados, la funcion es a grandes rasgos un 
% member que recibe 2 listas y comprueba si la lista 1 esta dentro de la lista 2.
comprobar_prerrequisitos([], _).
comprobar_prerrequisitos([Prerrequisito|Resto], Aprobados):-
    member(Prerrequisito, Aprobados),
    comprobar_prerrequisitos(Resto, Aprobados).
...
es_prerrequisito(Pre, Asignatura):-
    prerrequisitos(Asignatura, Prerrequisitos),
    member(Elemento, Prerrequisitos), % se saca una asignatura cualquiera de la lista de Prerrequisitos y se verifica si es prerrequisito o no con recursion
    es_prerrequisito(Pre, Elemento),!.
